...
--   sys.availability_groups.cluster_type_desc   (2017, read by 021)
--   sys.dm_hadr_cluster.quorum_state            (read by 020)
...
SELECT CONVERT(varchar(23), SYSDATETIME(), 126)                   AS [collected_at],
       ...
       (SELECT COUNT(*) FROM sys.dm_hadr_database_replica_states) AS [always_on.database_replicas],
       ...
       CONVERT(int, SERVERPROPERTY('IsClustered'))                AS [cluster.is_clustered],
       /* Mirroring, counted here and detailed below. */
       (SELECT COUNT(*) FROM sys.database_mirroring AS m
         WHERE m.mirroring_guid IS NOT NULL)                      AS [counts.mirrored_databases]
...
/* Every endpoint that carries availability traffic, plus its TCP state. The
   LEFT JOIN is deliberate: an endpoint defined and not listening has no row in
   sys.tcp_endpoints' port view on some versions, and dropping it would hide the
   one that explains a partner that cannot connect. */
