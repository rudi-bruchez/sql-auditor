...
// loop is the wizard, minus its terminal. It returns the process's exit code.
//
...
// only kind this repository has. With these three parameters the loop is
// exercisable end to end — keystrokes, ticks, progress, panics and the exit
// code — with a channel and three closures.
//
// hook is what starts the connection, the probe, the collection and the ticker
// when the state enters a step that needs them, and stops them when it leaves.
// It cannot be an event, because an event is applied by this goroutine and a
// producer sending from here would block on a channel this goroutine is no
// longer reading; and it cannot go through draw, which sees rendered lines and
// not the state. So it is a callback, invoked on THIS goroutine after every
// apply and before the frame is painted — which is also what makes it free of
// locks: everything it touches is reached from here and nowhere else. It is nil
// for every test of the loop itself.
func loop(events <-chan event, draw func([]string), size func() (int, int), s State, hook func(prev, next State)) int {
	...
	// only sensible answer, and the exit path still has the progress buffer to
	// flush.
	return code
}
