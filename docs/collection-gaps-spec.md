...
A second audit pass on two SQL Server 2016 SP1 instances, one a transactional
publisher and distributor, the other its push subscriber. The replication
collectors specified earlier had been implemented between the two passes, so
this section is mostly about how they behaved on a real topology. Entry bar is
unchanged: each item cost this audit a sentence it could not write.
...
`041.replication-publisher`, `042.replication-distribution` and
`044.replication-counters` all worked. On the publisher they returned the
publication, its 136 articles, the distribution retention, five agents, the row
counts of `MSrepl_commands` and `MSrepl_transactions`, and both latency legs.
Nothing to fix there.

`043.replication-subscriber` returned `applies: 0` and every count at zero on
an instance that is unquestionably a push subscriber: its subscription database
carries 272 `sp_MSins_*` and `sp_MSupd_*` procedures generated by the snapshot,
and the distributor names it in its own agent rows.

The gate was `sys.databases.is_subscribed`, which is 0 on that database. That
flag is documented as unreliable elsewhere in the corpus, and the collector had
reintroduced the same test.

**What it cost.** The subscriber's own archive could not establish that it is a
subscriber. The topology had to be reconstructed from the publisher's archive
and from a procedure listing, which is exactly the detour the collector was
written to remove.

**Closed.** `043.replication-subscriber.sql` now recognises a subscription
database by `MSreplication_subscriptions`, which the snapshot always creates, or
by the generated apply procedures, and records in `applies_source` which test
answered. `is_subscribed` is kept last and still projected, because a flag that
disagrees with the evidence is itself worth seeing in the archive.

### 14. Article rows did not carry `schema_option` — closed

`041.replication-publisher` reads `sysarticles` and projects `artid`, `name`,
`dest_table`, `objid` and `pubid`. The row is already read; one column is
missing.

`schema_option` is the bitmask that says what the snapshot recreates on the
subscriber: clustered index, nonclustered indexes, declarative referential
integrity, extended properties, collation. Without it, nothing in the archive
says whether the subscriber's indexes come from replication or were created by
hand afterwards.

**What it cost.** The audit could not answer that question, and the client had
to answer it by email. The answer inverted the recommendation: with nonclustered
index copying off, every index on the subscriber is hand-made, nothing
maintains alignment with the publisher, and a reinitialisation drops all of
them. That is a finding worth a paragraph in the report, and it came from an
email rather than from the archive.

**Closed.** `041.replication-publisher.sql` now projects `schema_option` and
decodes the bits that decide what the snapshot recreates, so the archive answers
the question without an email.

### 15. The 13.0.5026 floor excluded SQL Server 2016 SP1 entirely — closed for VLFs

Four collectors are gated at 13.0.5026: `10.system/014.cpu-topology`,
`10.system/051.version-store`, `20.databases/023.log-vlf` and
`20.databases/024.log-stats`. Both instances audited run 13.0.4451 and
13.0.4457, so all four were skipped, on both, on both passes.

The floor is correct for the DMVs those collectors read. The consequence is
not: SP1 is still common on estates that are audited precisely because they are
old, and the audit loses the log file diagnosis on the version where it is most
likely to be needed.

**What it cost.** Three topics went out of collection on both instances
(`excessive-vlf-count`, `excessive-vlfs`, `numa-affinity-misconfigured`), and
the report had to write that it could not tell hardware NUMA from soft-NUMA. The
client's question for this audit was about transaction log management, so the
VLF count was the one number the analysis most wanted and did not have.

**Closed for the log, still open for the rest.** `023.log-vlf.sql` has dropped
its `@min_version` and now asks whether `sys.dm_db_log_info` exists rather than
computing arithmetic on a version string, falling back to `DBCC LOGINFO` and
recording which path answered. The three other collectors gated at 13.0.5026
still return nothing below SP2, and `014.cpu-topology` is the one that costs a
sentence: without it, hardware NUMA cannot be told from soft-NUMA, and no MAXDOP
argument can lean on memory locality.

The shape to reuse for them is the one that worked here: ask whether the object
...
`public` are still not collected. Four topics stay out of collection on both
instances, one of them rated high (`least-privilege-database-roles`), and
`msdb.dbo.sysalerts` is still absent, which keeps
`severity-19-25-and-io-error-alerts-missing` out as well.
...
Consolidating the optimizer's missing-index suggestions produces candidates with
several equality columns, and the order of those columns is the one decision
that matters most in an index: most selective first. The archive cannot settle
it. The analysis tooling now orders them by accumulated index advantage and says
in its own output that this is not a selectivity order.

**What it cost.** Three of the three consolidated candidates on the busiest
table of the audited subscriber carry an undecided key order, and the report had
to recommend the index without being able to fix its leading column.

`sys.dm_db_stats_properties` is already called by `70.schema/090.statistics.sql`,
but it does not return the density vector: it carries rows, steps, sample size
and modification counter, none of which describe distribution. The vector comes
from `DBCC SHOW_STATISTICS ... WITH DENSITY_VECTOR`, or from
`sys.dm_db_stats_histogram` on 2016 and later for the leading column.

Either is enough, and either beats the alternative: a `COUNT(DISTINCT)` per
